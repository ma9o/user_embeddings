...
def read_single_zst_ndjson_chunked(
    ...
    try:
            ...
            with dctx.stream_reader(fh) as reader:
                ...
                for line in text_reader:
                    ...
                    try:
                        ...
                        logger.warning(
                            f"Skipping line {line_num} due to JSON decode error in {file_basename}: {json_err} - Line: {line[:100]}..."
                        )
                    # UnicodeDecodeError should be handled by errors='replace' now
                    except Exception as e:
                        logger.warning(
                            f"Skipping line {line_num} due to unexpected error ({type(e).__name__}) in {file_basename}: {e} - Line: {line[:100]}..."
                        )
        ...
        logger.warning(
            f"Zstandard decompression error in {filepath}: {zstd_err}, skipping file."
        )
    except Exception as e:
        logger.warning(
            f"Failed to read {filepath} due to unexpected error ({type(e).__name__}): {e}, skipping file."
        )
...
def read_single_zst_ndjson(filepath: str) -> Iterable[Dict[str, Any]]:
    ...
    start_time = time.time()
    file_basename = os.path.basename(filepath)
    logger.info(f"Starting extraction for: {file_basename}")  # Initial message
            ...
            with dctx.stream_reader(fh) as reader:
                ...
                for line in text_reader:
                    ...
                    try:
                        ...
                        logger.warning(
                            f"Skipping line {line_num} due to Unicode decode error in {file_basename}: {unicode_err} - Line: {line[:100]}..."
                        )
                    except Exception as e:
...
def read_zst_ndjson_files(
    ...
    logger.info(f"Found {len(filepaths)} files matching '{file_pattern}' in {directory_path}")
    ...
    if meta is None and columns is not None:
        ...
        logger.warning(
            "No meta provided, constructing basic meta from columns with object dtype. Performance might suffer."
        )
    ...
    if columns and meta is None:
        ...
        if missing_cols:
            logger.warning(
                f"Requested columns not found after DataFrame creation: {missing_cols}"
            )
